...
```{r}
library(gridExtra)
ggsave(grid.arrange(before_day_density, after_day_density, nrow = 2), file="day_density_before.png", dpi=600, width=11, height=8.5)
```
...
preds1 <- cbind(ants, boosted_predictions = boost_preds)
...
boost_preds <- predict(boost_ants1, newdata=for_predict)
preds2 <- cbind(ants, boosted_predictions = boost_preds)
```
