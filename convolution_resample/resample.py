...
with open(resample_params_file, 'r') as f:
    sensor_params = json.load(f)[sensor_name]
...
plt.plot(wavelengths, hyperspectral_data[row, col, :], label='Hyperspectral')
plt.plot(band_centers, resampled[row, col, :], 'o-', label=f'Resampled ({sensor_name})')
plt.xlabel("Wavelength (nm)")
plt.ylabel("Reflectance")
plt.title("Spectral Convolution Example")
plt.legend()
plt.grid(True)
plt.savefig('downsampled.png')

resampled_hdr_path = f"{sensor_name.replace(' ', '_').lower()}_resampled.hdr"
