        ...
        config_dict = {
            ...
            "topo": {
                ...
                "calc_mask": [
                    ...
                    [
                        "ancillary",
                        {"name": "slope", "min": np.radians(5), "max": "+inf"},
                    ],
            ...
            "num_cpus": 1,
            "resample": False,
            "resampler": {"type": "cubic", "out_waves": [], "out_fwhm": []},
        }
...
def process_hdf5_with_neon2envi(image_path, site_code):
    command = [
        "/opt/conda/envs/macrosystems/bin/python",
        "neon2envi2.py",
        "--output_dir",
        "output/",
        "--site_code",
...
def get_spectral_data_and_wavelengths(filename, row_step, col_step):
    ...
    colrange = np.arange(
        0, envi.columns
    ).tolist()  # Adjusted to use envi.columns for dynamic range
...
def load_spectra(filenames, row_step=6, col_step=1):
    ...
    for filename in filenames:
        try:
            spectral_data, wavelengths = get_spectral_data_and_wavelengths(
                filename, row_step, col_step
            )
            results[filename] = {
...
def extract_overlapping_layers_to_2d_dataframe(raster_path, gpkg_path):
    ...
    with rasterio.open(raster_path) as src:
        ...
        polygons["intersects"] = polygons.geometry.apply(
            lambda geom: geom.intersects(box(*raster_bounds))
        )
...
def rasterize_polygons_to_match_envi(
    gpkg_path, existing_raster_path, output_raster_path, attribute=None
):
    # Load polygons
    polygons = gpd.read_file(gpkg_path)
    ...
    rasterized_polygons = rasterize(
        shapes=(
            ...
            for geom, value in zip(
                ...
                (
                    polygons[attribute]
                    if attribute and attribute in polygons.columns
                    else polygons.index
                ),
            )
        ),
...
def prepare_spectral_data(spectral_data, wavelengths):
    ...
    long_df = pd.melt(
        pd.DataFrame(spectral_data).transpose(),
        var_name="band",
...
def reshape_spectra(results, index):
    ...
    if index < 4:
        ...
        long_df = pd.melt(
            df_spectral_data, var_name="wavelength_nm", value_name="reflectance"
        )
        waves = pd.DataFrame(wavelengths, columns=["wavelength_nm"])
        waves["band"] = range(len(waves))
        ...
        first_key = (
            keys[index].replace("export/resample_", "").replace(".img", "")
        )  # Modified here to remove "export/"
        merged_data["sensor"] = first_key
        merged_data = merged_data.reindex(
            columns=["sensor", "band", "wavelength_nm", "reflectance", "pixel"]
        )

        length = len(merged_data)
        sequence = np.arange(0, 1071)  # Creates an array [1, 2, ..., 999]
        repeated_sequence = np.resize(
            sequence, length
        )  # Resize the sequence to match the DataFrame's length

        merged_data["pixel"] = repeated_sequence  # Add the column to your DataFrame
        merged_data["sensor_band"] = (
            merged_data["sensor"].astype(str) + "_" + merged_data["band"].astype(str)
        )
        return merged_data
    else:
        merged_data = prepare_spectral_data(spectral_data, wavelengths)
        # Add a label column to the merged data
        first_key = keys[index].replace(
            "export/ENVI__corrected_0", "hyperspectral_corrected"
        )
...
def plot_spectral_data(df, highlight_pixel):
    ...
    for idx in unique_indices:
        ...
        plt.plot(
            subset["wavelength_nm"], subset["reflectance"], alpha=0.05, color="blue"
        )
    ...
    if (highlighted_subset["reflectance"] == -9999).all() or highlighted_subset[
        "reflectance"
    ].isna().all():
        print(f"Warning: Pixel {highlight_pixel} data is entirely -9999 or NaN.")

    plt.plot(
        highlighted_subset["wavelength_nm"],
        highlighted_subset["reflectance"],
        color="red",
        linewidth=10,
        label=f"Pixel {highlight_pixel}",
    )
...
def plot_each_sensor_with_highlight(
    concatenated_sensors, highlight_pixel, save_path=None
):
    ...
    for i, sensor in enumerate(sensors):
        ...
        highlighted_subset = df[df["pixel"] == highlight_pixel]
        if (
            not highlighted_subset.empty
            and not highlighted_subset["reflectance"].isna().all()
        ):
            axs[i].plot(
                highlighted_subset["wavelength_nm"],
                highlighted_subset["reflectance"],
...
def plot_with_highlighted_sensors(
    concatenated_sensors, highlight_pixels, save_path=None
):
    ...
    concatenated_sensors = concatenated_sensors[
        concatenated_sensors["wavelength_nm"] > 0
    ]
    concatenated_sensors["reflectance"] = concatenated_sensors["reflectance"].replace(
        -9999, np.nan
    )

    # Plotting hyperspectral corrected data in blue
    hyperspectral_corrected = concatenated_sensors[
        concatenated_sensors["sensor"] == "hyperspectral_corrected"
    ]
    ...
    for sensor in concatenated_sensors["sensor"].unique():
        if (
            sensor != "hyperspectral_corrected"
        ):  # Exclude hyperspectral corrected data from red lines
...
def boosted_quantile_plot(
    data,
    num_lines=10,
    title="Hyperspectral Corrected Predictions by Alpha",
    save_path=None,
):
    ...
    for alpha_value in alpha_values:
        ...
        plt.plot(
            data["wavelength_nm"],
            data[alpha_col],
            label=f"Probability {alpha_col}",
            alpha=adjusted_alpha,
            color="red",
            linewidth=adjusted_alpha * 6,
        )
...
def boosted_quantile_plot_by_sensor(
    data,
    num_lines=10,
    title="Hyperspectral Corrected Predictions by Alpha",
    save_path=None,
    ...
    for i, sensor in enumerate(sensors, 1):
        ...
        for alpha_value in alpha_values:
            ...
            plt.plot(
                sensor_data["wavelength_nm"],
                sensor_data[alpha_col],
                label=f"Probability {adjusted_label}",
                alpha=adjusted_alpha,
                color="red",
                linewidth=adjusted_alpha * 6,
            )
...
def show_rgb(file_paths, r=660, g=550, b=440):
    ...
    for file_path, ax in zip(file_paths, axs.flatten()):
        ...
        rgb = np.stack([hy_obj.get_wave(r), hy_obj.get_wave(g), hy_obj.get_wave(b)])
        ...
        # Normalize
        rgb = (rgb - np.nanmin(rgb, axis=(0, 1))) / (
            np.nanmax(rgb, axis=(0, 1)) - np.nanmin(rgb, axis=(0, 1))
        )
...
def clean_csv_files_in_subfolders(base_folder, chunk_size=100000, suffix="_no_NaN"):
    ...
    for subdir in subdirectories:
        try:
            ...
            original_csv_pattern = os.path.join(
                subdir, "*_spectral_data_all_sensors.csv"
            )
            ...
            for original_csv in original_csv_files:
                ...
                if total_lines <= 0:
                    print(
                        f"The file {original_csv} is empty or has only a header. Skipping.\n"
                    )
                    continue
                ...
                with (
                    pd.read_csv(original_csv, chunksize=chunk_size) as reader,
                    open(cleaned_csv, "w") as output_file,
                ):
                    for i, chunk in enumerate(
                        tqdm(reader, total=num_chunks, desc=f"Cleaning {base_name}")
                    ):
                        ...
                        mode = (
                            "w" if i == 0 else "a"
                        )  # Write mode for the first chunk, append for others
...
def validate_output_files(base_folder):
    ...
    subdirectories = [
        ...
        for d in os.listdir(base_folder)
        if os.path.isdir(os.path.join(base_folder, d))
        and not d.startswith(".ipynb_checkpoints")
    ]
...
def load_and_combine_rasters(raster_paths):
    ...
    for path in raster_paths:
        ...
        chunk = processor.get_chunk_from_extent(
            corrections=["some_correction"], resample=False
        )
        chunks.append(chunk)
    combined_array = np.concatenate(
        chunks, axis=0
    )  # Combine along the first axis (bands)
    return combined_array


def process_and_flatten_array(
    array,
    json_dir="Resampling",
    original_bands=426,
    corrected_bands=426,
    original_wavelengths=None,
    corrected_wavelengths=None,
    folder_name=None,
    map_info=None,
):
    """
    ...
    if len(array.shape) != 3:
        raise ValueError(
            "Input array must be 3-dimensional. Expected (bands, rows, cols)."
        )
    ...
    # Corrected bands
    if (
        corrected_wavelengths is not None
        and len(corrected_wavelengths) >= corrected_bands
    ):
        for i in range(corrected_bands):
            wl = corrected_wavelengths[i]
            band_names.append(f"Corrected_band_{i+1}_wl_{wl}nm")
    elif (
        original_wavelengths is not None
        and len(original_wavelengths) >= corrected_bands
    ):
        for i in range(corrected_bands):
            wl = original_wavelengths[i]
            band_names.append(f"Corrected_band_{i+1}_wl_{wl}nm")
    ...
    if sensor_bands_assigned < remaining_bands:
        ...
        print(
            f"[DEBUG] {extra} leftover bands have no matching sensors/wavelengths in JSON. Naming them generically."
        )
...
def control_function(directory):
    ...
    if os.path.isfile(hdr_file):
        ...
        print(
            f"No HDR file found at {hdr_file}. Will use generic band names and no geocoords."
        )
    ...
    try:
        ...
        print(
            f"DataFrame shape after flattening for directory {directory}: {df_processed.shape}"
        )
    except ValueError as e:
        print(f"ValueError encountered during processing of {directory}: {e}")
        print(
            "Check the number of bands vs. the expected column names in process_and_flatten_array()."
        )
        return
    except Exception as e:
