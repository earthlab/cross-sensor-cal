...
@lru_cache(maxsize=1)
def _resolve_calc_resample_coeffs():
    """Locate HyTools' ``calc_resample_coeffs`` implementation."""

    import_hytools()

    try:
        module = import_module("hytools.transform.resampling")
    except ModuleNotFoundError as exc:  # pragma: no cover - handled when HyTools incomplete
        raise HyToolsNotAvailable(
            "HyTools is installed but missing transform.resampling. Ensure the package is "
            "built with resampling support."
        ) from exc

    if not hasattr(module, "calc_resample_coeffs"):
        raise HyToolsNotAvailable(
            "HyTools does not expose 'calc_resample_coeffs'. Update HyTools to a version "
            "that includes resampling utilities."
        )

    return getattr(module, "calc_resample_coeffs")


def calc_resample_coeffs(in_wave, in_fwhm, out_waves, out_fwhm):
    """Proxy to HyTools' ``calc_resample_coeffs`` with a friendly error."""

    return _resolve_calc_resample_coeffs()(in_wave, in_fwhm, out_waves, out_fwhm)
