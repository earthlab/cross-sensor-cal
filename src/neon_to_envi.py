...
def neon_to_envi_task(hy_obj, output_dir):
    ...
    if envi_output_file.path.exists():
        print(f'Skipping {envi_output_file.file_path} ENVI conversion, file already exists')
        return

    hy_obj.load_data()

    # Initialize ENVI writer with header from HyTools object
    writer = WriteENVI(envi_output_file.file_path, hy_obj.get_header())
    ...
    while not iterator.complete:
        ...
        if iterator.complete:
            writer.close()
    print(f"Exported {neon_input_file.name} to ENVI format at {envi_output_file.name}.")
...
def flight_lines_to_envi(input_dir: str, output_dir: str):
    neon_reflectance_files = NEONReflectanceFile.find_in_directory(Path(input_dir))
