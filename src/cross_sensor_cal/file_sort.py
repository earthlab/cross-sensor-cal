...
def categorize_file(file_obj: DataFile) -> str:
    ...
    if isinstance(
        ...
        ) or (isinstance(file_obj, MaskedFileMixin) and file_obj.is_masked)
    ...
    # BRDF mask files are inherently masks
        ...
        file_obj, (NEONReflectanceBRDFMaskENVIFile, NEONReflectanceBRDFMaskENVIHDRFile)
    ):
        return "Reflectance_Masked"
...
def generate_file_move_list(
        ...
        try:
            ...
            if file_class in suffix_classes:
                ...
                if file_class == NEONReflectanceResampledENVIFile:
                    found_files = file_class.find_all_sensors_in_directory(
                        base_path, suffix="envi"
                    )
                else:
                    ...
                    for sensor in SensorType:
                        ...
                        try:
                            sensor_files = file_class.find_in_directory(
                                base_path, sensor=sensor_name, suffix="envi"
                            )
    ...
    csv_path = os.path.join(
        destination_folder, "sorted_files", "envi_file_move_list.csv"
    )
    ...
    if len(df) > 0:
        ...
        category_counts = (
            df["Destination Path"]
            .apply(lambda x: x.split("/envi/")[1].split("/")[0])
            .value_counts()
        )
