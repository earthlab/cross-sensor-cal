...
def apply_resampler(hy_obj, data):
    ...
    if hy_obj.resampler["type"] == "gaussian":
        ...
        if "resample_coeffs" not in hy_obj.ancillary:
            ...
            resample_coeffs = calc_resample_coeffs(
                in_wave,
                in_fwhm,
        ...
        interp_func = interp1d(
            hy_obj.wavelengths[~hy_obj.bad_bands],
            data,
...
class resampler_hy_obj:
    ...
    def __init__(self, sensor_type, json_file, hdr_path):
        ...
        fwhm_value = 5
        self.fwhm = np.array(
            self.get_in_fwhm(num_of_bands, fwhm_value), dtype=np.float32
        )
        self.ancillary = {}
        ...
        self.out_wave = np.array(
            band_parameters[sensor_type]["wavelengths"], dtype=np.float32
        )
        self.out_fwhm = np.array(
            band_parameters[sensor_type]["fwhms"], dtype=np.float32
        )
    ...
    def create_header_info(self, hdr_path):
        ...
        map_info = header.get(
            "map info", None
        )  # 'map info' contains georeferencing details
        coordinate_system_string = header.get(
            "coordinate system string", None
        )  # Optional CRS details
    ...
    def save_envi_data(data, header_info, output_filename):
        ...
        spectral.envi.save_image(
            output_filename, data, metadata=header_info, force=True
        )
...
def translate_to_sensor(
    sensor_type, json_file, hdr_path, resampling_file_path: str, output_file_path: str
):
    # create the obj
    ...
    resampler_config_obj.save_envi_data(
        data=final_resampled_data,
        header_info=header_info,
        output_filename=output_file_path,
    )


def translate_to_other_sensors(
    brdf_corrected_img_file: NEONReflectanceBRDFCorrectedENVIFile,
):
    ...
    brdf_corrected_header_file = (
        NEONReflectanceBRDFCorrectedENVIHDRFile.from_components(
    ...
    for sensor_type in sensor_types:
        resampled_dir = Path(
            os.path.join(
                os.path.dirname(brdf_corrected_img_file.directory),
        ...
        try:
            translate_to_sensor(
                ...
                json_file,
                brdf_corrected_header_file,
                brdf_corrected_img_file.file_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the JSON file with Landsat band parameters",
    )
    parser.add_argument(
        "--hdr_path", type=str, required=True, help="Path to the HDR file"
    )
    parser.add_argument(
        "--sensor_type", type=str, required=True, help="Give the type of sensor"
    )
    parser.add_argument(
        "--resampling_file_path",
        type=str,
        required=True,
        help="Path to the resampling file",
    )
    parser.add_argument(
        "--output_path", type=str, required=True, help="Path for the output file"
    )
