...
def mask_raster_with_polygons(
    ...
    def assign_crs(
        raster, polygons, raster_crs_override=None, polygons_crs_override=None
    ):
        """
    ...
    def clip_polygons(raster, polygons_aligned):
        ...
        raster_bounds_geom = gpd.GeoDataFrame(
            {"geometry": [box(*raster.bounds)]}, crs=raster.crs
        )
        clipped_polygons = gpd.overlay(
            polygons_aligned, raster_bounds_geom, how="intersection"
        )
    ...
    def plot_results(raster, masked_data, nodata, clipped_polygons, plot_path):
        ...
        """
        print(
            f"Masked data stats: Min={masked_data.min()}, Max={masked_data.max()}, Unique={np.unique(masked_data)}"
        )
        ...
        clipped_polygons.plot(
            ax=axes[1, 0], facecolor="none", edgecolor="blue", linewidth=0.5
        )
    ...
    if raster.transform.is_identity:
        print(
            "Warning: Raster has an identity transform. Masked data may not be georeferenced correctly."
        )

    masked_raster_path = save_masked_raster(
        envi_file, masked_data, raster.nodata, raster
    )
...
def find_raster_files(directory):
    ...
    filtered_files = [
        ...
        if (
            (
                "_reflectance" in os.path.basename(file)
                or "_envi" in os.path.basename(file)
