...
def topo_and_brdf_correction(config_file: str):
    ...
    if config_dict["file_type"] == "envi":
        ...
        ray.get(
            [
                actor.read_file.remote(
                    image, config_dict["file_type"], anc_files[str(image)]
                )
    ...
    # bad bands
    ray.get(
        [actor.create_bad_bands.remote(config_dict["bad_bands"]) for actor in actors]
    )
    ...
    brdf_corrected_file = NEONReflectanceBRDFCorrectedENVIFile.from_components(
        ...
        suffix=config_dict["export"][
            "suffix"
        ],  # e.g., "_corrected_envi" -> becomes "envi"
    ...
    if config_dict["export"]["coeffs"] and config_dict["corrections"]:
        ...
        ray.get(
            [actor.do.remote(export_coeffs, config_dict["export"]) for actor in actors]
        )
...
def apply_corrections(hy_obj, config_dict):
    ...
    if not brdf_corrected_file.path.exists():
        ...
        iterator = hy_obj.iterate(
            by="line", corrections=hy_obj.corrections, resample=config_dict["resample"]
        )
    ...
    if config_dict["export"]["masks"]:
        ...
        if not brdf_corrected_masked_file.path.exists():
            print(
                f"📦 Writing correction masks: {brdf_corrected_masked_file.file_path}"
            )
            masks = []
            mask_names = []
            for correction in config_dict["corrections"]:
...
def generate_correction_configs_for_directory(
    ...
    anc_files = NEONReflectanceAncillaryENVIFile.find_in_directory(
    ...
    ancillary_mapping = {
        band: [str(anc_file.file_path), i] for i, band in enumerate(ancillary_bands)
    }
    ...
    config_dict = {
        ...
        "anc_files": {str(reflectance_file.file_path): ancillary_mapping},
        "export": {
            "coeffs": True,
            "image": True,
            "masks": True,
        ...
        "num_cpus": 4,
        "resample": False,
        "resampler": {"type": "cubic", "out_waves": [450, 550, 650], "out_fwhm": []},
    }
...
def generate_config_json(directory: str, num_cpus: int = 8):
    ...
    refl_paths = glob.glob(
        os.path.join(directory, "**", "*_reflectance_envi.img"), recursive=True
    )
    # exclude corrected and ancillary
    refl_paths = [
        p
        for p in refl_paths
        if "ancillary" not in p.lower() and "corrected" not in p.lower()
    ]
    ...
    for img_path in refl_paths:
        ...
        ancandidates = NEONReflectanceAncillaryENVIFile.find_in_directory(
            reflectance_file.directory
        )
        if not ancandidates:
            print(
                f"⚠️ No ancillary files found for {reflectance_file.file_path}; skipping."
            )
            continue
        ...
        ancillary_mapping = {
            name: [str(ancillary.file_path), i]
            for i, name in enumerate(ancillary_band_names)
        }
